...
export default async function FavorisPage({ searchParams }: Props) {
  ...
  // !inner => seuls les listings ayant un favori de cet utilisateur sont retournés
  let query = supabase
    .from("listings")
    .select(
      `id, title, city, price, surface, photos, favorites!inner(user_id)`,
      { count: "exact" },
    )
    .eq("status", "active")
  ...
  function pageHref(targetPage: number): string {
    ...
    return `/favoris?${params.toString()}`;
  }
    ...
    <main className="mx-auto w-full max-w-7xl px-6 py-10 space-y-8 animate-fade-in-up">
      ...
      {error ? (
        ...
        <p className="text-sm text-muted-foreground">
          {count === 0 && !trimmedCity && !validType && !priceMin && !priceMax && !surfaceMin && !surfaceMax && !rooms
          ...
          {totalPages > 1 && (
            <Pagination>
              <PaginationContent>
                {page > 1 && (
                  <PaginationItem>
                    <PaginationPrevious href={pageHref(page - 1)} text="Précédent" />
                  </PaginationItem>
                )}
                  ...
                  .map((item, idx) =>
                    item === "ellipsis" ? (
                      ...
                      <PaginationItem key={item}>
                        <PaginationLink href={pageHref(item)} isActive={item === page}>
                          {item}
                        </PaginationLink>
                      </PaginationItem>
                    ),
                ...
                {page < totalPages && (
                  <PaginationItem>
                    <PaginationNext href={pageHref(page + 1)} text="Suivant" />
                  </PaginationItem>
                )}
              </PaginationContent>
            </Pagination>
